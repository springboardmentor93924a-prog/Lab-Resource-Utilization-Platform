...
import {
  ...
  Repeat, ChevronRight, RotateCcw, Sparkles, FileText
} from "lucide-react";
...
export function BookEquipmentForm({ equipment, onClose, onBooked, onSwitchToWaitlist, toast }) {
  ...
  useEffect(() => {
    bookingApi
      .getCurrentAgreement()
      .then((data) => {
        if (data) setAgreementConfig(data);
      })
      .catch((err) => {
        console.warn("Could not load backend agreement terms:", err);
      });
  }, []);
  ...
  const handleConfirmSubmit = async () => {
    if (!agreementChecked) {
      setError("Please review and accept the laboratory usage agreement before submitting.");
      return;
    }
    setError("");
    setSubmitting(true);
    ...
    try {
      const payload = {
        ...
        agreementVersion: agreementConfig?.version || "2026.1",
      };
        ...
        <div className="p-6">
          ...
          {step === "confirm_review" && (
            <div className="space-y-6">
              ...
              {/* Real Booking Usage & Safety Agreement */}
              <div className="rounded-2xl border border-blue-200 bg-blue-50/50 p-4 space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <FileText size={16} className="text-blue-600 shrink-0" />
                    <span className="text-xs font-bold text-slate-900">
                      {agreementConfig?.title || "Laboratory Resource Usage Agreement"}
                    </span>
                  </div>
                  {agreementConfig?.version && (
                    <span className="text-[10px] font-mono bg-blue-100 text-blue-800 px-2.5 py-0.5 rounded-full font-bold">
                      v{agreementConfig.version}
                    </span>
                  )}
                </div>

                <p className="text-[11px] text-slate-600 leading-relaxed">
                  {agreementConfig?.summary || "Standard institutional laboratory equipment usage policy and safety protocols."}
                </p>

                {agreementConfig?.terms && agreementConfig.terms.length > 0 && (
                  <ul className="text-[11px] text-slate-600 list-disc list-inside space-y-1 pl-1 bg-white/80 p-3 rounded-xl border border-blue-100">
                    {agreementConfig.terms.map((term, idx) => (
                      <li key={idx} className="leading-snug">{term}</li>
                    ))}
                  </ul>
                )}
                ...
                <label className="flex items-start gap-2.5 pt-1 cursor-pointer select-none">
                  ...
                  <span className="text-xs font-semibold text-slate-800 leading-snug">
                    I confirm that I have reviewed, understood, and agreed to adhere to all laboratory operating protocols, equipment safety SOPs, and usage terms.
                  </span>
                </label>
              </div>
