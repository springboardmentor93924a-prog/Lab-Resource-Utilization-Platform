...
public class ResourceSharingServiceImpl implements ResourceSharingService {
    ...
    public ResourceSharingRequest updateStatus(Long id, String status) {
        ...
        // other role reaching this method (only LAB_MANAGER and
        // DEPARTMENT_HEAD can, per the controller's @PreAuthorize —
        // INSTITUTION_ADMIN was removed from this action) is scoped to
        // their OWN institution, and that institution must be the
        // request's sender (the equipment owner). This is exactly what
        // lets a Dept Head/Lab Manager approve or reject a request that
        // came FROM another college: the request's sender is their own
        // institution (the one that owns the equipment), the receiver is
        // whichever college asked for access — same-college actors never
        // see a sender that isn't themselves, so this one check is what
