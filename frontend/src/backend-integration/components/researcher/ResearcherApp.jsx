...
export function ResearcherApp({ toast, onLogout }) {
  ...
  const goToReportIssue = (equipmentOrBooking) => {
    setReportIssueEquipment(equipmentOrBooking || null);
  ...
  if (view === "dashboard") {
    ...
    content = <MyBookings onReportIssue={goToReportIssue} toast={toast} />;
    ...
    <ResearcherLayout activeView={view} onNavigate={navigate} unreadCount={unreadCount} onLogout={handleLogout}>
      ...
      {bookingModalEquipment && (
        <BookEquipmentForm
          ...
          onSwitchToWaitlist={(eqWithPrefill) => {
            setBookingModalEquipment(null);
            setWaitlistModalEquipment(eqWithPrefill);
          }}
          toast={toast}
        />
      )}
