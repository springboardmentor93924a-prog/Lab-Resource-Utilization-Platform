...
public class WorkOrderController {
    ...
    @GetMapping("/my")
    public ResponseEntity<List<WorkOrderResponse>> getMyAssignedWorkOrders(Authentication authentication) {
        return ResponseEntity.ok(workOrderService.getMyAssignedWorkOrders(authentication.getName()));
    }

    @GetMapping("/equipment/{equipmentId}")
    public ResponseEntity<List<WorkOrderResponse>> getWorkOrdersByEquipment(@PathVariable Long equipmentId) {
        return ResponseEntity.ok(workOrderService.getWorkOrdersByEquipment(equipmentId));
    }

    @PutMapping("/{id}/assign")
    public ResponseEntity<WorkOrderResponse> assignTechnician(@PathVariable Integer id,
                                                              @Valid @RequestBody WorkOrderAssignRequest request,
                                                              Authentication authentication) {
        return ResponseEntity.ok(workOrderService.assignTechnician(id, request, authentication.getName()));
    }

    @PutMapping("/{id}/complete")
    public ResponseEntity<WorkOrderResponse> markComplete(@PathVariable Integer id,
                                                          Authentication authentication) {
        return ResponseEntity.ok(workOrderService.markComplete(id, authentication.getName()));
    }
}