        ...
        <div className="p-8 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
              ...
              <Detail icon={Users} label="Capacity per Slot" value={equipment.capacityPerSlot != null ? `${equipment.capacityPerSlot} user(s)` : "1 user"} />
              <Detail
          ...
          {equipment.status === "UNDER_MAINTENANCE" && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-semibold text-red-800 flex items-center gap-2.5">
              ...
              <span>This equipment is currently undergoing maintenance and is unavailable for reservations.</span>
            </div>
          )}

          {equipment.status === "OUT_OF_SERVICE" && (
            <div className="rounded-xl border border-slate-300 bg-slate-100 p-4 text-xs font-semibold text-slate-700 flex items-center gap-2.5">
              <AlertTriangle size={18} className="text-slate-500 shrink-0" />
              <span>This equipment is currently marked out of service and cannot be reserved.</span>
            </div>
          )}
          ...
          <div className="pt-6 border-t border-slate-100 flex flex-wrap items-center gap-4">
            {canBook && (
              <button
                ...
                className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-8 py-3.5 shadow-md hover:shadow-lg transition-all flex items-center gap-2"
              >
            ...
            {canWaitlist && (
              <button
                ...
                className="rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold px-8 py-3.5 shadow-md hover:shadow-lg transition-all flex items-center gap-2"
              >
                <Users size={16} /> Join Waitlist
              </button>
            )}
            {!canBook && !canWaitlist && equipment.status !== "RETIRED" && (
              <span className="rounded-xl border border-slate-200 bg-slate-50 text-slate-500 text-sm font-semibold px-6 py-3.5">
                Unavailable for Booking ({equipment.status?.replace(/_/g, " ")})
              </span>
            )}
          </div>
        </div>
